...
/***/ function(module, exports) {
	...
	document.getElementById('files').onchange = function () {
	  ...
	  for (var i = 0, file_num = this.files.length; i < file_num; i++) {
	    ...
	    FILEs.map(function (e, index) {
	      if (e.name == _this.files[i].name) {
	        FILEs[i] = e;
	        same = true;
	        return;
	      }
	...
	function add_files(file_name) {
	  ...
	  new_li.innerHTML = file_name + '<span><a href="javascript:void(0);" onclick="del_file(this,\'' + file_name + '\')">删除</a></span><br/>';
